...
const userdb = JSON.parse(fs.readFileSync('./json/db_user.json', 'UTF-8'))
...
server.post('/auth/register', (req, res) => {
  ...
  fs.readFile('./json/db_user.json', (err, datas) => {
    if (err) {
      const status = 401
      const message = err
      res.status(status).json({ status, message })
    ...
    fs.writeFile('./json/db_user.json', JSON.stringify(data), (err, result) => {
      // WRITE
      if (err) {
        const status = 401
        const message = err
...
// Login to one of the users from ./json/db_user.json
...
// Update user from ./json/db_user.json after login
